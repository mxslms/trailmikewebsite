...
I want models that run **where the trail is** — not only in a cloud notebook. The near-term project is real-time object detection on an **NVIDIA Jetson Orin Nano**, with a path toward wildlife detection. Same app on a home GPU box and on the Jetson; different containers, shared code.
